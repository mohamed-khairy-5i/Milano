...
const POS: React.FC<POSProps> = ({ isRTL }) => {
  const { products, currency, addInvoice } = useData();
  ...
  const handlePay = () => {
      ...
      // Create Invoice
      addInvoice({
          number: `POS-${Date.now().toString().substr(-4)}`,
          date: new Date().toISOString().split('T')[0],
          dueDate: new Date().toISOString().split('T')[0],
          contactName: isRTL ? 'عميل عام' : 'Walk-in Customer',
          contactId: '0', // Default walk-in
          total: total,
          paidAmount: total,
          remainingAmount: 0,
          tax: tax,
  ...
  const handlePrint = () => {
      ...
      const htmlContent = `
        ...
        <html dir="${direction}">
        <head>
            <title>${isRTL ? 'إيصال' : 'Receipt'}</title>
            <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700&display=swap" rel="stylesheet">
            <style>
                body { font-family: 'Cairo', sans-serif; padding: 20px; max-width: 300px; margin: 0 auto; background: #fff; }
                .header { text-align: center; border-bottom: 1px dashed #000; padding-bottom: 10px; margin-bottom: 10px; }
                .title { font-weight: bold; font-size: 18px; margin: 5px 0; }
                ...
                .print-btn { width: 100%; padding: 10px; background: #000; color: #fff; border: none; margin-bottom: 20px; cursor: pointer; border-radius: 4px; }
        ...
        <body>
             ...
             <div class="header">
                <div class="title">Milano Store</div>
             ...
             <div class="items">
                ${cart.map(item => `
                    <div class="item">
                        ...
                        <span>${(item.priceSell * item.quantity).toLocaleString()}</span>
                    </div>
                `).join('')}
             </div>
             <div class="totals">
             ...
             <div class="footer">
                ${isRTL ? 'شكراً لزيارتكم' : 'Thank you for visiting'}
             </div>
        </body>
        </html>
      `;
      ...
      <div className="w-full lg:w-96 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col h-full">
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
            ...
            <div className="grid grid-cols-2 gap-2">
                <button 
                    onClick={handlePay}
                    className="flex items-center justify-center gap-2 py-2.5 px-4 bg-primary text-white rounded-lg hover:bg-blue-700 transition-colors font-medium"
                >
                    <CreditCard size={18} />
                    {isRTL ? 'دفع' : 'Pay Now'}
                </button>
                <button 
                    onClick={handlePrint}
                    className="flex items-center justify-center gap-2 py-2.5 px-4 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors font-medium"
                >
                    <Printer size={18} />
                    {isRTL ? 'طباعة' : 'Print'}
                </button>
